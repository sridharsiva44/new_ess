...
export default function Odform() {
    ...
    const handleApplyLeave = () => {
        ...
        const SearchData = {
            ...
            toDate: timeOutValue,
            noofDays: noofdays,
            remarks: data.reasonbox
        }

        postCommonData("/ESS-Java/api/emplyee/addOdDetails/", SearchData)
            .then((res) => {
                if (res.status === 200) {
                    toast.success('OD Successfully Applied!', {
    ...
    return (
        <Paper sx={{ margin: 2 }}>
            <Grid>
                <Grid>
                    <Typography>OD REQUEST</Typography>
                    <Grid spacing={2} item>
